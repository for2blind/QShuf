...
class MetricCollector:
    ...
    def collect_pod_metrics(self, start_time, end_time):
        ...
        for metric_name in self.mdict_pod.keys():
            query_result = self.prom.custom_query_range(self.mdict_pod[metric_name], end_time=end_time, start_time=start_time, step="5s")
    ...
    def collect_deploy_metrics(self, start_time, end_time):
        ...
        for metric_name in self.deploy_metrics.keys():
            query_result = self.prom.custom_query_range(self.deploy_metrics[metric_name], end_time=end_time, start_time=start_time, step="5s")
            for m in query_result:
                data_t = pd.json_normalize(m)
                dk_tmp = pd.DataFrame(columns=self.columns_pod)             
                for i,r in data_t.iterrows():
...
class OpenFaasCollector:
    def __init__(self,fun_name):
        self.prom = PrometheusConnect(url="http://prometheus.example", disable_ssl=False) 
        self.sum_metrics = {
...
if __name__ == "__main__":
    ...
    exp="ES1"
    record_csv=f'evaluation_record_{exp}.csv'
    record = pd.read_csv(record_csv, header=0) 
    mc = MetricCollector()   
    interval=10 
    wula_base_path=f"QShuf/evaluation/wula_result/{exp}/"
    for i,r in record.iterrows():
        start_time_rc = r['start_time']
        print(f'collecting {exp} {r["wrkname"]}, {r["start_time"]}, {r["end_time"]}, total time: {(r["end_time"]-r["start_time"])/3600}')
        ...
        for csv_file in get_csv_files_in_bottom_directory(csv):
            ...
            prom_data_path = f'../system/{exp}/{wrkname}/{scheduler}/{start_time_rc}/'
            ...
            if not collect_record[(collect_record['uuid'] == evaluation_id) & (collect_record['cv'] == cv_value)]['collected'].eq(1).any():
                node_perf_all, gpu_perf_all = mc.collect_all_metrics(start_time=start_time,end_time=end_time)
                gpu_perf_all['evaluation_id'] = evaluation_id
                gpu_perf_all['cv_mu_duration'] = cv_mu_duration
                gpu_perf_all.to_csv(f'{prom_data_path}/../gpu_{evaluation_id}_{cv_value}.csv',header=False)

                pod_metrics_all = mc.collect_pod_metrics(start_time=start_time,end_time=end_time)
                pod_metrics_all['evaluation_id'] = evaluation_id
                pod_metrics_all['cv_mu_duration'] = cv_mu_duration
                pod_metrics_all.to_csv(f'{prom_data_path}/../pod_{evaluation_id}_{cv_value}.csv',header=False)

                sum_metrics_all = mc.collect_sum_metrics(start_time=start_time,end_time=end_time)
                sum_metrics_all['evaluation_id'] = evaluation_id
                sum_metrics_all['cv_mu_duration'] = cv_mu_duration
                sum_metrics_all.to_csv(f'{prom_data_path}/../sum_{evaluation_id}_{cv_value}.csv',header=False)
                collect_record = collect_record._append({'uuid': evaluation_id, 'cv': cv_value, 'collected': 1}, ignore_index=True)
            ...
            for fun_name in fun_name_list: 
                ...
                scale_df = openfaas_prom.collect_sum_metrics(start_time=start_time,end_time=end_time)
