    ...
    def setup_routes(self):
        ...
        async def tocpu():
            ...
            with torch.inference_mode():
                infer_device = "cpu"
                x = torch.load('/data/model/openfaas/openfaas-fn/wholegraphs/whisper-large-v2/whisper-large-v2-wholemod-inputs.pt', map_location=infer_device)    
                model = model.to("cpu")
                # print(model(x))
...
def load_model_and_update_status(loaded_app):
    ...
    with torch.inference_mode():
        ...
        x = torch.load('/data/model/openfaas/openfaas-fn/wholegraphs/whisper-large-v2/whisper-large-v2-wholemod-inputs.pt', map_location=infer_device)    
        # start_time= time.time()    
        model= torch.load(f'/data/model/openfaas/openfaas-fn/wholegraphs/whisper-large-v2/whisper-large-v2-wholemod.pt',map_location=infer_device)
