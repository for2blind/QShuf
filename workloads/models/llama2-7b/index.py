    ...
    def setup_routes(self):
        ...
        async def tocpu():
            ...
            with torch.inference_mode():
                # self.ready = False  # set loaded to False since too slow to serving
                # start_time = time.time()
                model.to("cpu")
                # print("time to move model to cpu 1:", time.time() - start_time)
                # start_time = time.time()
                model.cpu()
                # print("time to move model to cpu 2:", time.time() - start_time)
        ...
        async def tocuda():
            ...
            with torch.inference_mode():
                ...
                # print("time to move x to cuda:", time.time() - start_time)
                # start_time = time.time()
                model.to("cuda")
                # print("time to move model to cuda 1:", time.time() - start_time)
                # start_time = time.time()
                model.cuda()
                # print("time to move model to cuda :", time.time() - start_time)
                infer_device = "cuda"
                torch.cuda.empty_cache()
            end_time = time.time()
            self.loaded = True 
...
def load_model_and_update_status(loaded_app):    
    with torch.inference_mode():
        ...
        x = torch.load("/data/model/openfaas/openfaas-fn/wholegraphs/llama2-7b/llama2-7b-wholemod-inputs.pt", map_location=infer_device)
        # torch.cuda.empty_cache()
        # start_time = time.time()
        model= torch.load(f'/data/model/openfaas/openfaas-fn/wholegraphs/llama2-7b/llama2-7b-wholemod.pt',map_location=infer_device)
        # print("loaded time:", time.time() - start_time," on ",infer_device)
        # torch.cuda.empty_cache()
        # output = model(**x)
        # cuda 8.61s cpu 7.15s tocuda 2.37s tocpu 6.53s
