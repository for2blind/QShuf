...
def load_model_and_update_status(loaded_app):    
    with torch.inference_mode():
        ...
        x = torch.load("/sharing/model/opt-1.3b-pt/opt-1.3b.pt", map_location=infer_device)
        torch.cuda.empty_cache()
        start_time = time.time()
        model= torch.load(f'/sharing/model/opt-1.3b-pt/opt-1.3b-input.pt',map_location=infer_device)
...
async def call_handler(path: str, request: Request):
    start_time = time.time()
    try:
        # print("adding inference task to queue")
