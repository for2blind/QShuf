...
class FunctionManager:
    ...
    def deploy_function(self, function_name, gateway_address):
        ...
        if function_path:
            ...
            os.system(f"faas-cli deploy --filter {function_name} --gateway http://{gateway_address}/ -f config.yml")
        else:
    ...
    def delete_function(self, function_name, gateway_address):
        ...
        if function_path:
            os.chdir(os.path.dirname(os.path.abspath(__file__)))
            os.chdir(function_path)
            os.system(f"faas-cli delete --filter {function_name} --gateway http://{gateway_address}/ -f config.yml")
        else:
            print(f"Function {function_name} not found in the available functions.")
